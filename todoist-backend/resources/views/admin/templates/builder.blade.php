...
<div class="card">
    <div class="row">
        <h3 style="margin:0;">Template Wizard Builder</h3>
        <div>
            <a class="btn" href="{{ route('admin.templates.create', ['per_page' => $perPage]) }}">New Template</a>
            <a class="btn" href="{{ route('admin.panel', ['section' => 'templates', 'per_page' => $perPage]) }}">Back to List</a>
        </div>
    </div>
    <p class="muted">Manage phases and steps with media fields (image/video/audio/pdf), links, and paragraph content.</p>
    ...
    <div class="grid-2">
        ...
        <input id="tpl-description" class="field" placeholder="Short description">
    </div>
    ...
    <div class="row" style="justify-content:flex-end; flex-wrap:wrap;">
        ...
        <button id="tpl-save-template" class="btn primary" type="button">Save Template Wizard</button>
    </div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function normalizeTemplate(raw) {
    ...
    if (!phases.length) {
      var fallbackSteps = (raw.steps || []).map(function (title, idx) {
        return { id: 'step-' + (idx + 1), title: title, type: 'text', paragraph: '', media: { image: '', video: '', audio: '', pdf: '' }, links: [] };
      });
      phases = [{ id: 'phase-1', title: 'Phase 1', steps: fallbackSteps }];
    }
    phases = phases.map(function (phase, pIdx) {
      var steps = (phase.steps || []).map(function (step, sIdx) {
        return {
          id: step.id || ('p' + (pIdx + 1) + '-s' + (sIdx + 1)),
          title: step.title || ('Step ' + (sIdx + 1)),
          type: step.type || 'text',
          paragraph: step.paragraph || '',
          media: Object.assign({ image: '', video: '', audio: '', pdf: '' }, step.media || {}),
          links: Array.isArray(step.links) ? step.links : []
        };
      });
      if (!steps.length) {
        steps.push({ id: 'p' + (pIdx + 1) + '-s1', title: 'Step 1', type: 'text', paragraph: '', media: { image: '', video: '', audio: '', pdf: '' }, links: [] });
      }
      return { id: phase.id || ('phase-' + (pIdx + 1)), title: phase.title || ('Phase ' + (pIdx + 1)), steps: steps };
    });
    return { id: raw.id, name: raw.name || '', category: raw.category || '', description: raw.description || '', phases: phases };
  }
  ...
  function renderPhaseSelect() {
    ...
    phaseSelect.innerHTML = state.phases.map(function (phase, idx) { return '<option value="' + idx + '">' + phase.title + '</option>'; }).join('');
  ...
  function renderStepSelect() {
    ...
    stepSelect.innerHTML = (phase.steps || []).map(function (step, idx) { return '<option value="' + idx + '">' + step.title + '</option>'; }).join('');
  ...
  window.AdminApi.getOne('templates', templateId).then(function (template) {
    state = normalizeTemplate(template);
    renderAll();
  }).catch(function (error) {
    statusEl.textContent = 'Failed to load template: ' + error.message;
  });
  ...
  document.getElementById('tpl-save-template').addEventListener('click', function () {
    saveCurrentStepFromForm();
    statusEl.textContent = 'Saving template wizard...';
    window.AdminApi.update('templates', templateId, state).then(function () {
      statusEl.textContent = 'Template wizard saved successfully.';
      renderPreview();
    }).catch(function (error) {
